...
项目当前已完成 **Phase 2**：PostgreSQL、显式 Schema 初始化、sqlc、事务、Argon2id 和用户核心能力已经接入。Redis、登录、Token 和认证会话将在 Phase 3 加入；当前仍不会开放未认证的用户管理 HTTP 接口。
...
先在当前 Shell 中安全设置 `GOBA_DATABASE_PASSWORD`，不要把实际值写入仓库。首次启动显式初始化数据库：
...
Compose 编排 GoBA Slim 与 PostgreSQL；`db-init` 只在 `tools` Profile 下由部署方显式执行，`serve` 不会自动修改 Schema。Redis 将在 Phase 3 接入。镜像以非 root 用户运行，生产 TLS 应由 Nginx、Ingress 或 API Gateway 终止。
...
internal/platform/           配置、数据库、日志、HTTP 和健康检查
